...
function SocialIcon({ href, label, children }: { href: string; label: string; children: React.ReactNode }) {
  return (
    <a
      ...
      aria-label={label}
      className="flex h-9 w-9 items-center justify-center rounded-full text-sage-600 transition duration-200 hover:-translate-y-0.5 hover:bg-sage-100 hover:text-sage-800 dark:text-sage-300 dark:hover:bg-white/10 dark:hover:text-sage-100"
    >
      {children}
    </a>
  );
  ...
  return (
    <div className="flex items-center gap-4">
      <div className="relative shrink-0">
        <OrganicBlob tone="mint" className="-inset-3" />
        <div className="relative z-10">
          <TeamAvatar slug={person.slug} name={person.name} />
        </div>
      </div>
...
export function MadeBy({ people }: { people: Person[] }) {
  return (
    <div>
      ...
      <div className="mt-7 flex flex-col gap-8 sm:flex-row sm:items-center sm:gap-14">
        {people.map((p) => (
